...
const UserRegistration = require('../models/user/user-registration');
const UserLogon = require('../models/user/user-logon');
const AccountController = require('../controllers/account');
const User = require('../models/user/user');
const UserSession = require('../models/user/user-session');
const mailer = require('../models/mailer');
const ApiResponse = require('../models/api-response');
const ApiMessages = require('../models/api-messages');
const UserPasswordReset = require('../models/user/user-pwd-reset');
const UserPasswordResetFinal = require('../models/user/user-pwd-reset-final');
const UserPasswordResetRemember = require('../models/user/user-pwd-reset-remember');
...
router.route('/account/register').post((req, res) => {
  ...
  if (!apiResponse1.success) {
    ...
    res.send(new ApiResponse({ success: false, extras: apiResponse1.extras }));
    return;
  }
...
router.route('/account/user')
  .get((req, res) => {
    if (!req.session || !req.session.userProfileModel) {
      ...
      res.send(new ApiResponse({
        success: true, extras: {
          user: req.session.userProfileModel.nickname,
        }
      }))
    }
  });
